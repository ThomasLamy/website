...
get_pubs <- function() {
    ...
    pubs <- read_excel(here("data", "cv.xlsx"), sheet ="pubs")
    ...
    pubs$url_scholar <- ifelse(
      ...
      glue::glue('https://scholar.google.com/citations?view_op=view_citation&hl=en&user=i3F3Q2UAAAAJ&citation_for_view=i3F3Q2UAAAAJ:{pubs$id_scholar}')
    )
    return(pubs)
}
